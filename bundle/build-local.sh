...
echo "  docker run --rm -it --name express-compute-installer -v ~/.aws:/root/.aws:ro --entrypoint bash -e AWS_REGION=us-east-1 ${IMAGE_TAG}"
echo ""
echo "Run interactively (env vars / EC2 / CloudShell):"
echo "  docker run --rm -it --name express-compute-installer -e AWS_ACCESS_KEY_ID -e AWS_SECRET_ACCESS_KEY -e AWS_SESSION_TOKEN -e AWS_REGION=us-east-1 --entrypoint bash ${IMAGE_TAG}"
